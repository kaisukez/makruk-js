...
import { importFen, INITIAL_FEN, EMPTY_FEN } from "0x88/fen/importer"
...
describe("round-trip tests (import → export → import)", () => {
    test("should maintain INITIAL_FEN through round-trip", () => {
        ...
        const state2 = importFen(exported)

        expect(exported).toBe(INITIAL_FEN)
        expect(state2.activeColor).toBe(state1.activeColor)
    ...
    test("should maintain EMPTY_FEN through round-trip", () => {
        const state1 = importFen(EMPTY_FEN)
        const exported = exportFen(state1)
        const state2 = importFen(exported)
    ...
    test("should maintain FEN with countdown through round-trip", () => {
        ...
        const state1 = importFen(originalFen)
        const exported = exportFen(state1)
        const state2 = importFen(exported)
...
describe("edge cases", () => {
    ...
    test("should handle FEN with both active colors", () => {
        ...
        const stateWhite = importFen(fenWhite)
        const stateBlack = importFen(fenBlack)

        expect(stateWhite.activeColor).toBe(Color.WHITE)
        expect(stateBlack.activeColor).toBe(Color.BLACK)

        expect(exportFen(stateWhite)).toBe(fenWhite)
        expect(exportFen(stateBlack)).toBe(fenBlack)
    })

    test("should correctly handle empty countdown history", () => {
        const fen = "4k3/8/8/8/8/8/8/4K3 w 1"
        const state = importFen(fen)

        expect(state.countdownHistory).toEqual([])
        expect(Array.isArray(state.countdownHistory)).toBe(true)
    })
