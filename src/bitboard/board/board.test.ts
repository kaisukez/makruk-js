...
describe("createEmptyBoardState", () => {
    test("should create an empty bitboard state", () => {
        ...
        expect(state.whiteRua).toBe(EMPTY_MASK)
        expect(state.blackRua).toBe(EMPTY_MASK)
        expect(state.whiteMa).toBe(EMPTY_MASK)
        expect(state.blackMa).toBe(EMPTY_MASK)
        expect(state.whiteThon).toBe(EMPTY_MASK)
        expect(state.blackThon).toBe(EMPTY_MASK)
        expect(state.whiteMet).toBe(EMPTY_MASK)
        expect(state.blackMet).toBe(EMPTY_MASK)
        expect(state.whiteKhun).toBe(EMPTY_MASK)
        expect(state.blackKhun).toBe(EMPTY_MASK)
    })
})
...
describe("squareIndexToMask64Square", () => {
    test("should convert 0x88 square to bitboard square", () => {
        expect(squareIndexToMask64Square(0x00 as SquareIndex)).toBe(0) // a1
        expect(squareIndexToMask64Square(0x07 as SquareIndex)).toBe(7) // h1
        expect(squareIndexToMask64Square(0x70 as SquareIndex)).toBe(56) // a8
        expect(squareIndexToMask64Square(0x77 as SquareIndex)).toBe(63) // h8
    })
})

describe("squareToIndex", () => {
    test("should convert bitboard square to 0x88 square", () => {
        expect(squareToIndex(0)).toBe(0x00) // a1
        expect(squareToIndex(7)).toBe(0x07) // h1
        expect(squareToIndex(56)).toBe(0x70) // a8
        expect(squareToIndex(63)).toBe(0x77) // h8
    })
})
...
describe("getLSB", () => {
    test("should get least significant bit index", () => {
        expect(getLSB(EMPTY_MASK)).toBe(-1)
...
describe("popLSB", () => {
    test("should pop least significant bit", () => {
        ...
        expect(result1.bb).toBe(EMPTY_MASK)
...
describe("getMSB", () => {
    test("should get most significant bit index", () => {
        expect(getMSB(EMPTY_MASK)).toBe(-1)
...
describe("getPieceMask64", () => {
    test("should get bitboard for white pieces", () => {
        ...
        expect(getPieceMask64(state, Color.WHITE, Piece.BIA)).toBe(1n << 8n)
        expect(getPieceMask64(state, Color.WHITE, Piece.FLIPPED_BIA)).toBe(1n << 9n)
        expect(getPieceMask64(state, Color.WHITE, Piece.MA)).toBe(1n << 10n)
        expect(getPieceMask64(state, Color.WHITE, Piece.THON)).toBe(1n << 11n)
        expect(getPieceMask64(state, Color.WHITE, Piece.MET)).toBe(1n << 12n)
        expect(getPieceMask64(state, Color.WHITE, Piece.RUA)).toBe(1n << 13n)
        expect(getPieceMask64(state, Color.WHITE, Piece.KHUN)).toBe(1n << 14n)
    })
    ...
    test("should get bitboard for black pieces", () => {
        ...
        expect(getPieceMask64(state, Color.BLACK, Piece.BIA)).toBe(1n << 48n)
        expect(getPieceMask64(state, Color.BLACK, Piece.FLIPPED_BIA)).toBe(1n << 49n)
        expect(getPieceMask64(state, Color.BLACK, Piece.MA)).toBe(1n << 50n)
        expect(getPieceMask64(state, Color.BLACK, Piece.THON)).toBe(1n << 51n)
        expect(getPieceMask64(state, Color.BLACK, Piece.MET)).toBe(1n << 52n)
        expect(getPieceMask64(state, Color.BLACK, Piece.RUA)).toBe(1n << 53n)
        expect(getPieceMask64(state, Color.BLACK, Piece.KHUN)).toBe(1n << 54n)
    })
...
describe("printMask64", () => {
    ...
    test("should print empty bitboard", () => {
        const result = printMask64(EMPTY_MASK)
        expect(typeof result).toBe("string")
        expect(result).toContain(". ")
